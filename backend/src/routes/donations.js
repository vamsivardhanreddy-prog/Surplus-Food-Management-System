...
import { notifyNearbyNgos, sendNotificationToUser } from "../lib/notifications.js";
...
async function enrichDonation(donation, ngoLat, ngoLng) {
...
router.get("/nearby", requireRole("ngo"), async (req, res) => {
  const user = req.user;
  ...
  const radiusKm = parseFloat(req.query.radiusKm ?? "10") || 10;
...
router.post("/", requireRole("donator"), async (req, res) => {
  const user = req.user;
  const {
    title,
    foodItems,
  ...
  // Notify NGOs within 5km that new food is available (any verified NGO can claim if within 0.5km of their location)
...
router.post("/:id/claim", requireRole("ngo"), async (req, res) => {
  const user = req.user;
